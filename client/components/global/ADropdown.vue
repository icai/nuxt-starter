<template>
  <component :is="tag" v-click-outside="closeDropDown" v-on="listeners" :class="[{ show: isOpen }]">
    <slot :is-open="isOpen" name="toggle">
      <a class="nav-link dropdown-toggle" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Pages</a>
    </slot>
...
export default {
  ...
  created() {
    this.listeners = Object.assign(
      {
        [this.dataToggle == 'hover' ? 'mouseenter' : 'click']: this.toggleDropDown,
        [this.dataToggle == 'hover' ? 'mouseleave' : 'blur']: this.closeDropDown
      },
      this.$listeners
    )
  },
