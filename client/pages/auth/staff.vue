        ...
        <div class="page-login--content-main content-wrapper">
          ...
          <div class="page-login--formwrap">
            ...
            <div class="page-login--form form-container">
              <el-form ref="loginForm" label-position="top" :rules="rules" v-model="model" @submit.prevent="onSubmit" size="large">
                ...
                <el-form-item prop="verify">
                  <el-input type="text" v-model="model.verify" @keyup.enter="onSubmit" placeholder="Verify Code">
                    <template #append>
                      <img id="yzm" :src="vcode" @click="vertifyCode" />
                    </template>
                  </el-input>
                </el-form-item>
                <el-button  @click="onSubmit" size="large" type="primary" class="button-login"> Login </el-button>
              </el-form>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const onSubmit = async () => {
      try {
        const { email, password, isRemember, verify } = model.value;
        // validate
        if (!email) {
          return this.$notify({
            title: 'Notify',
        ...
        await this.$store
          ...
          .then((res) => {
            return this.$auth.redirect('admin', true);
          })
          .catch((err) => {
            vertifyCode();
            this.$notify({
